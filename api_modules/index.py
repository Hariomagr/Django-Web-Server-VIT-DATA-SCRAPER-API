    ...
    if(len(gsid)>1):
        if(len(gsid[0].split("="))>1):
            if(len((gsid[0].split("="))[1].split(";"))>0):
                ...
                captcha = ""
                x=os.path.join(BASE_DIR, "api_modules")
                #data = open(x+'\\static\\bitmaps.json').read()
                #bitmap = json.load(data)
                with open(x+'\\static\\bitmaps.json', "r") as f:
                    bitmap = json.load(f)
